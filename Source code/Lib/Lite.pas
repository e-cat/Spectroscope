...
const
  CustomColorCount = 16; // ChooseColor dialog
...
type
  ...
  TCustomColorsArray = array[0..CustomColorCount - 1] of TColorRef;
...
function CopyStr(const Source: string; var Dest; MaxLen: Integer): Integer;

function Hex(Value, Digits: Integer): string;
...
function RegGetValue(Key: HKEY; const Name: string; Typ: Cardinal;
  var Buffer; BufSize: Cardinal): LongWord;
function RegGetIntValue(Key: HKEY; const Name: string; DefValue: Cardinal = 0): LongWord;
function RegGetBoolValue(Key: HKEY; const Name: string; DefValue: Boolean = False): Boolean;
function RegGetStrValue(Key: HKEY; const Name: string; const DefValue: string = ''): string;
function RegSetValue(Key: HKEY; const Name: string; Value: Cardinal): Boolean; overload;
function RegSetValue(Key: HKEY; const Name: string; Value: Boolean): Boolean; overload;
function RegSetValue(Key: HKEY; const Name: string; const Value: string): Boolean; overload;
function SetStartup(Startup: Boolean; User: Boolean = True;
...
function CreateSimplePopupMenu(const Items: array of string): HMENU;
procedure SetMenuCheck(Menu: HMENU; ID: Integer; Checked: Boolean);
...
function FullScreenApp: Boolean;
...
function MutexAppProc(AppProc: TProc): Boolean;

procedure WindowedAppProc(var WndClass: WNDCLASS; WndStyle: Cardinal = WS_POPUP;
  WndExStyle: Cardinal = 0; const WndName: string = '');
...
begin
  ...
  begin
    ...
    if Result > MaxLen then
      Result := MaxLen;
    Move(Pointer(Source)^, Dest, Result);
    (PChar(@Dest) + Result)^ := #0;
    Inc(Result);
  end;
end;
...
function Hex(Value, Digits: Integer): string;
const
  BaseChars: array[Boolean] of Char = ('0', Chr(Ord('A') - 10));
var
  I, J: Integer;
begin
  ...
  begin
    J := Value and $F;
    Result[I] := Chr(Ord(BaseChars[J >= 10]) + J);
    Value := Value shr 4;
  end;
end;
...
begin
  ReadIniFile(ProgramName + '.ini', IniProc);
end;
...
begin
  Result := BufSize;
  if (RegQueryValueEx(Key, Pointer(Name), nil, @ActualType, @Buffer, @Result) <>
    ERROR_SUCCESS) or (ActualType <> Typ) then
    Result := 0;
end;

function RegGetIntValue(Key: HKEY; const Name: string;
  DefValue: Cardinal): LongWord;
begin
  if RegGetValue(Key, Name, REG_DWORD, Result, SizeOf(LongWord)) = 0 then
    Result := DefValue;
end;

function RegGetBoolValue(Key: HKEY; const Name: string;
  DefValue: Boolean): Boolean;
begin
  Result := RegGetIntValue(Key, Name, Ord(DefValue)) <> 0;
end;

function RegGetStrValue(Key: HKEY; const Name, DefValue: string): string;
var
  Len: Integer;
  Buf: array[0..$3FF] of Char;
begin
  Len := RegGetValue(Key, Name, REG_SZ, Buf, SizeOf(Buf));
  if Len > 0 then
...
function RegSetValue(Key: HKEY; const Name: string; Value: Cardinal): Boolean;
begin
  Result := RegSetValueEx(Key, Pointer(Name), 0, REG_DWORD, @Value, SizeOf(
    LongWord)) = ERROR_SUCCESS;
end;

function RegSetValue(Key: HKEY; const Name: string; Value: Boolean): Boolean;
begin
  Result := RegSetValue(Key, Name, Ord(Value = True));
end;

function RegSetValue(Key: HKEY; const Name: string;
  const Value: string): Boolean;
begin
  Result := RegSetValueEx(Key, Pointer(Name), 0, REG_SZ, PChar(Value), Length(
    Value) + 1) = ERROR_SUCCESS;
end;


function SetStartup(Startup, User: Boolean; const Params: string): Boolean;
const
  Keys: array[Boolean] of HKEY = (HKEY_LOCAL_MACHINE, HKEY_CURRENT_USER);
var
  Key: HKEY;
begin
  Result := False;
  if RegOpenKeyEx(Keys[User], 'Software\Microsoft\Windows\CurrentVersion\Run',
    0, KEY_SET_VALUE, Key) =
    ERROR_SUCCESS then
    try
      if Startup then
        Result := RegSetValue(Key, ProgramName, '"' + ParamStr(0) + '"' +
          Params)
      else
        Result := RegDeleteValue(Key, Pointer(ProgramName)) = ERROR_SUCCESS;
    finally
      RegCloseKey(Key);
    end;
end;
...
begin
  ...
  begin
    ...
    S := Items[I];
    if (S = '') or (S = '-') then
      Flags := Flags and MF_SEPARATOR;
    AppendMenu(Result, Flags, I, Pointer(S));
  end;
end;

procedure SetMenuCheck(Menu: HMENU; ID: Integer; Checked: Boolean);
const
  States: array[Boolean] of Cardinal = (MFS_UNCHECKED, MFS_CHECKED);
var
...
begin
  ...
  begin
    ...
    fMask := MIIM_STATE;
    fState := States[Checked];
  end;
  SetMenuItemInfo(Menu, ID, False, MenuItemInfo);
end;
...
var
  ...
  function IsProgMan(Wnd: HWND): Boolean;
  var
...
begin
  ...
  with Rect do
    Result := (Left = 0) and (Top = 0) and (Right = GetSystemMetrics(
      SM_CXSCREEN)) and (Bottom = GetSystemMetrics(SM_CYSCREEN)) and not
      IsProgMan(Wnd);
end;
...
begin
  ...
  try
    Result := WaitForSingleObject(Mutex, 0) = WAIT_OBJECT_0;
    if Result then
      AppProc;
  finally
    CloseHandle(Mutex);
...
procedure WindowedAppProc(var WndClass: WNDCLASS;
  WndStyle, WndExStyle: Cardinal; const WndName: string);
var
  Msg: TMsg;
  Wnd: HWND;
begin
  ...
  Wnd := CreateWindowEx(WndExStyle, WndClass.lpszClassName, Pointer(WndName),
